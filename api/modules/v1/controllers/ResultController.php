...
class ResultController extends ActiveController
{
     ...
     */
    public function actionTimKiemTheoNgay(){
        $tuNgay = Yii::$app->request->post('tuNgay');
        $denNgay = Yii::$app->request->post('denNgay');
        if (!empty($denNgay) && !empty($tuNgay)){
            $datas = Result::find()->andWhere(['>=', 'postdate',$tuNgay])->andWhere(['<', 'postdate',$denNgay]);
        }else{
            $datas = Result::find()->all();
        }
        $datasArray=[];
        foreach ($datas as $data){
            $datas = new \api\modules\v1\models\Result();
            $datas->Id = $data->id;
            $datas->PostDate = $data->postdate;
